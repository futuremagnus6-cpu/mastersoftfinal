const mongoose = require('mongoose');

const shopSchema = new mongoose.Schema({
  name: { type: String, required: [true, 'Shop name is required'], trim: true },
  businessType: {
    type: String,
    enum: [
      'medical_store', 'pharmacy', 'distributor', 'grocery_store',
      'supermarket', 'electronics_shop', 'mobile_shop', 'cosmetics_shop',
      'hardware_shop', 'riyansh_mlm', 'custom'
    ],
    required: [true, 'Business type is required'],
  },
  customBusinessType: { type: String, trim: true },
  gstin: { type: String, trim: true },
  pan: { type: String, trim: true },
  address: {
    line1: { type: String, required: true },
    line2: { type: String },
    city: { type: String, required: true },
    state: { type: String, required: true },
    pincode: { type: String, required: true },
    country: { type: String, default: 'India' },
  },
  contact: {
    phone: { type: String, required: true },
    email: { type: String, required: true },
    website: { type: String },
  },
  logo: { type: String },
  branding: {
    primaryColor: { type: String, default: '#2563eb' },
    secondaryColor: { type: String, default: '#1d4ed8' },
    accentColor: { type: String, default: '#f59e0b' },
    fontFamily: { type: String, default: 'Inter' },
    googleBusinessProfile: {
      enabled: { type: Boolean, default: false },
      businessName: { type: String, default: '' },
      category: { type: String, default: '' },
      description: { type: String, default: '' },
      googleMapsUrl: { type: String, default: '' },
      googleBusinessUrl: { type: String, default: '' },
      phone: { type: String, default: '' },
      website: { type: String, default: '' },
      businessHours: { type: String, default: '' },
      serviceArea: { type: String, default: '' },
      verified: { type: Boolean, default: false },
      plusCode: { type: String, default: '' },
      placeId: { type: String, default: '' },
      attributes: { type: String, default: '' },
    },
  },
  settings: {
    currency: { type: String, default: 'INR' },
    timezone: { type: String, default: 'Asia/Kolkata' },
    dateFormat: { type: String, default: 'DD/MM/YYYY' },
    language: { type: String, enum: ['en', 'hi', 'mr', 'gu', 'ta', 'te', 'kn', 'bn'], default: 'en' },
    taxInclusive: { type: Boolean, default: true },
    compositionScheme: { type: Boolean, default: false },
    offlinePos: { type: Boolean, default: false },
    darkMode: { type: Boolean, default: false },
    multiLanguage: { type: Boolean, default: false },
    autoBackup: { type: Boolean, default: false },
    backupTime: { type: String, default: '02:00' },
    defaultDiscount: { type: Number, default: 0, min: 0, max: 100 },
    invoicePrefix: { type: String, default: 'INV-' },
    orderPrefix: { type: String, default: 'ORD-' },
    invoiceTerms: { type: String, default: '1. Goods once sold will not be taken back.\n2. This is a computer-generated invoice.\n3. Subject to local jurisdiction.' },
    payment: {
      upiId: { type: String, default: '' },
      upiQrCode: { type: String, default: '' },
      defaultPaymentMethod: { type: String, enum: ['cash', 'upi', 'card', 'mixed', 'online'], default: 'cash' },
      autoCashDrawer: { type: Boolean, default: false },
      enableCashManagement: { type: Boolean, default: false },
    },
    customer: {
      autoCreateOnPos: { type: Boolean, default: true },
      defaultCreditLimit: { type: Number, default: 0 },
      allowCreditSales: { type: Boolean, default: false },
      requireGstForB2B: { type: Boolean, default: true },
      defaultDiscountPercent: { type: Number, default: 0 },
    },
    inventory: {
      defaultReorderLevel: { type: Number, default: 10 },
      stockValuationMethod: { type: String, enum: ['fifo', 'lifo', 'average'], default: 'average' },
      allowNegativeStock: { type: Boolean, default: false },
      enableSerialTracking: { type: Boolean, default: false },
      enableBatchTracking: { type: Boolean, default: false },
    },
    pos: {
      defaultView: { type: String, enum: ['grid', 'list'], default: 'grid' },
      showQuickKeys: { type: Boolean, default: true },
      enableHoldOrder: { type: Boolean, default: true },
      requireCashierSelect: { type: Boolean, default: false },
      defaultCustomer: { type: String, default: 'Walk-in Customer' },
      showRecentOrders: { type: Boolean, default: true },
      productCardSize: { type: String, enum: ['small', 'medium', 'large'], default: 'medium' },
      showProductImage: { type: Boolean, default: true },
      showProductSku: { type: Boolean, default: false },
      enableQuickSearch: { type: Boolean, default: true },
    },
    communication: {
      whatsappBusinessPhone: { type: String, default: '' },
      whatsappApiKey: { type: String, default: '' },
      whatsappReceiptTemplate: { type: String, default: '' },
      smsProvider: { type: String, default: '' },
      smsApiKey: { type: String, default: '' },
      smsSenderId: { type: String, default: '' },
      emailSmtpHost: { type: String, default: '' },
      emailSmtpPort: { type: Number, default: 587 },
      emailSmtpUser: { type: String, default: '' },
      emailSmtpPass: { type: String, default: '' },
      emailFromName: { type: String, default: '' },
      emailFromAddress: { type: String, default: '' },
    },
    alertConfig: {
      lowStockThreshold: { type: Number, default: 10 },
      expiryWarningDays: { type: Number, default: 30 },
      dailySalesReport: { type: Boolean, default: true },
      weeklyReport: { type: Boolean, default: true },
      monthlyReport: { type: Boolean, default: false },
      paymentReminders: { type: Boolean, default: false },
      backupReminders: { type: Boolean, default: true },
    },
    backupRetentionDays: { type: Number, default: 30 },
  },
  printConfig: {
    thermal: {
      paperWidth: { type: String, default: '80mm' },
      showLogo: { type: Boolean, default: true },
      storeName: { type: String, default: '' },
      tagline: { type: String, default: '' },
      headerAlignment: { type: String, enum: ['left', 'center', 'right'], default: 'center' },
      footerMessage: { type: String, default: 'Thank you for your purchase!' },
      showGstin: { type: Boolean, default: true },
      showCustomer: { type: Boolean, default: true },
      showPayments: { type: Boolean, default: true },
      showDiscount: { type: Boolean, default: true },
      showBatchNumber: { type: Boolean, default: false },
      showExpiryDate: { type: Boolean, default: false },
      showBarcode: { type: Boolean, default: false },
      showQrCode: { type: Boolean, default: false },
      qrCodeType: { type: String, enum: ['upi', 'invoice', 'review', 'whatsapp'], default: 'upi' },
      showCashierName: { type: Boolean, default: false },
      showBranchName: { type: Boolean, default: false },
      showTokenNumber: { type: Boolean, default: false },
      printCopies: { type: Number, default: 1, min: 1, max: 10 },
      customerCopy: { type: Boolean, default: true },
      merchantCopy: { type: Boolean, default: true },
      autoPrintAfterPayment: { type: Boolean, default: false },
      printPreview: { type: Boolean, default: true },
    },
    standard: {
      invoiceTitle: { type: String, default: 'Tax Invoice' },
      showLogo: { type: Boolean, default: true },
      showCompanyDetails: { type: Boolean, default: true },
      showCustomerSection: { type: Boolean, default: true },
      showBillingAddress: { type: Boolean, default: true },
      showShippingAddress: { type: Boolean, default: true },
      showCustomerGstin: { type: Boolean, default: true },
      showCustomerPhone: { type: Boolean, default: true },
      showCustomerEmail: { type: Boolean, default: false },
      showCustomerMembership: { type: Boolean, default: false },
      productColumns: {
        image: { type: Boolean, default: false },
        sku: { type: Boolean, default: false },
        barcode: { type: Boolean, default: false },
        hsn: { type: Boolean, default: true },
        batch: { type: Boolean, default: false },
        expiry: { type: Boolean, default: false },
        mrp: { type: Boolean, default: false },
        rate: { type: Boolean, default: true },
        discount: { type: Boolean, default: true },
        gst: { type: Boolean, default: true },
        total: { type: Boolean, default: true },
      },
      showTerms: { type: Boolean, default: true },
      termsText: { type: String, default: '1. Goods once sold will not be taken back.\n2. This is a computer-generated invoice.\n3. Subject to local jurisdiction.' },
      showSignature: { type: Boolean, default: true },
      showGstSummary: { type: Boolean, default: true },
      showAmountInWords: { type: Boolean, default: true },
      showDiscountDetails: { type: Boolean, default: true },
      showWatermark: { type: Boolean, default: false },
      watermarkText: { type: String, default: 'PAID' },
      watermarkType: { type: String, enum: ['paid', 'duplicate', 'cancelled', 'draft'], default: 'paid' },
      footerMessage: { type: String, default: '' },
      showQrCode: { type: Boolean, default: false },
      qrCodeType: { type: String, enum: ['invoice', 'upi', 'review', 'whatsapp', 'website'], default: 'invoice' },
      digitalSignature: { type: Boolean, default: false },
    },
    pdf: {
      orientation: { type: String, enum: ['portrait', 'landscape'], default: 'portrait' },
      margin: { type: Number, default: 10 },
      fontFamily: { type: String, default: 'Segoe UI' },
      fontSize: { type: Number, default: 12 },
      headerHeight: { type: Number, default: 40 },
      footerHeight: { type: Number, default: 30 },
    },
  },
  features: {
    pos: { type: Boolean, default: true },
    inventory: { type: Boolean, default: true },
    crm: { type: Boolean, default: false },
    suppliers: { type: Boolean, default: false },
    purchases: { type: Boolean, default: false },
    expenses: { type: Boolean, default: false },
    employees: { type: Boolean, default: false },
    multiBranch: { type: Boolean, default: false },
    ecommerce: { type: Boolean, default: false },
    customerPortal: { type: Boolean, default: false },
    barcodeScanner: { type: Boolean, default: false },
    thermalPrinter: { type: Boolean, default: false },
    whatsappNotifications: { type: Boolean, default: false },
    emailNotifications: { type: Boolean, default: true },
    lowStockAlerts: { type: Boolean, default: true },
    expiryAlerts: { type: Boolean, default: true },
    gstModule: { type: Boolean, default: true },
    apiAccess: { type: Boolean, default: false },
    referralSystem: { type: Boolean, default: false },
    affiliateSystem: { type: Boolean, default: false },
    aiForecasting: { type: Boolean, default: false },
    customerSupport: { type: Boolean, default: false },
  },
  limits: {
    maxUsers: { type: Number, default: 5 },
    maxProducts: { type: Number, default: 1000 },
    maxBranches: { type: Number, default: 1 },
    maxStorage: { type: Number, default: 5 }, // GB
  },
  subscription: {
    plan: { type: mongoose.Schema.Types.ObjectId, ref: 'SubscriptionPlan' },
    status: {
      type: String,
      enum: ['active', 'trial', 'expired', 'suspended', 'cancelled'],
      default: 'trial',
    },
    trialEndsAt: { type: Date },
    currentPeriodStart: { type: Date },
    currentPeriodEnd: { type: Date },
    razorpaySubscriptionId: { type: String },
    autoRenew: { type: Boolean, default: true },
    durationMonths: { type: Number, default: 1 }, // 1, 6, or 12
    gracePeriodEndsAt: { type: Date },
    suspendedAt: { type: Date },
  },
  usage: {
    currentUsers: { type: Number, default: 0 },
    currentProducts: { type: Number, default: 0 },
    currentBranches: { type: Number, default: 1 },
    currentStorage: { type: Number, default: 0 }, // MB
    totalOrders: { type: Number, default: 0 },
    totalRevenue: { type: Number, default: 0 },
    totalCustomers: { type: Number, default: 0 },
  },
  status: {
    type: String,
    enum: ['active', 'inactive', 'suspended', 'disabled'],
    default: 'active',
  },
  disabledByMaintenance: { type: Boolean, default: false },
  activatedAt: { type: Date },
  suspendedAt: { type: Date },
  createdBy: { type: mongoose.Schema.Types.ObjectId, ref: 'User' },
  updatedBy: { type: mongoose.Schema.Types.ObjectId, ref: 'User' },
}, {
  timestamps: true,
  toJSON: { virtuals: true },
  toObject: { virtuals: true },
});

shopSchema.index({ status: 1 });
shopSchema.index({ 'contact.email': 1 });
shopSchema.index({ 'subscription.status': 1 });
shopSchema.index({ gstin: 1 });

shopSchema.virtual('branches', {
  ref: 'Branch',
  localField: '_id',
  foreignField: 'shopId',
});

shopSchema.virtual('users', {
  ref: 'User',
  localField: '_id',
  foreignField: 'shopId',
});

module.exports = mongoose.model('Shop', shopSchema);
